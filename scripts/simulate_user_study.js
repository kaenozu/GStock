...
function generatePersona(id) {
    ...
    const experience = age < 25 ? (Math.random() > 0.8 ? 'Intermediate' : 'Beginner') :
        age > 50 ? (Math.random() > 0.5 ? 'Professional' : 'Beginner') :
            getRandomElement(['Beginner', 'Intermediate', 'Professional', 'Beginner']);
    // skewed towards beginner
    const style = getRandomElement(['Day Trader', 'Swing Trader', 'Long-term Investor', 'Quant', 'Gambler']);
...
async function main() {
    ...
    const reportPath = path_1.default.join(process.cwd(), 'user_simulation_report.json');
